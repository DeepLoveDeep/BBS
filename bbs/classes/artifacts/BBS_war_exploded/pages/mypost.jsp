...
<%
    ...
    String spath= request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+"/";
    String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
    if (session.getAttribute("username") == null){
        response.sendRedirect(spath+"login.jsp");
        return ;
    }
%>
            ...
            <ul class="list-group">
               ...
               <% ApplicationContext context = new ClassPathXmlApplicationContext("applicationContext.xml");
                ...
                <div class="list-group-item">
                    ...
                    <a href="javascript:void(0)" id="d<%=post.getId()%>"  style="float: right;color:red">删除</a>
                    <span id="<%=post.getId()%>" style="float: right">
                    <a href="<%=path%>/editpost.action?postId=<%=post.getId()%>" style="float: right">编辑&nbsp;</a>
                    <%if (post.getPostType() == 0 && !bestPostBiz.isExist(post.getId())) {%>
                    <a href="<%=path%>/applybest.action?postId=<%=post.getId()%>" style="float: right">申请精华贴&nbsp;</a></span>
                    <%} %>
                ...
                <script>
                    $(function(){
                       ...
                       if(flag==2){
                          $("#<%=post.getId()%>").html("<span style='color: red'>内容违规被驳回</span>&nbsp;");
                       }
                        if(flag==0){
                            $("#<%=post.getId()%>").html("<span style='color: orange'>内容涉及违规待审核中</span>&nbsp;");
                        }
                    $("#d<%=post.getId() %>").click(function(){
